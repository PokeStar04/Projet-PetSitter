...
<!-- animaux -->
<label>Chien</label>
<input type="checkbox" name="animaux" placeholder="chien" value="chien">
<label>Chat</label>
<input type="checkbox" name="animaux" placeholder="chat" value="chat">
<label>Lapin</label>
<input type="checkbox" name="animaux" placeholder="lapin" value="lapin"></br>
<label>Rongeur</label>
<input type="checkbox" name="animaux" placeholder="rongeur" value="rongeur">
<label>Furet</label>
<input type="checkbox" name="animaux" placeholder="furet" value="furet">
<label>Hérisson</label>
<input type="checkbox" name="animaux" placeholder="herisson" value="herisson"></br>
<label>Aquarium</label>
<input type="checkbox" name="animaux" placeholder="aquarium" value="aquarium">
<label>Oiseaux</label>
<input type="checkbox" name="animaux" placeholder="oiseaux" value="oiseaux">
<label>Reptiles</label>
<input type="checkbox" name="animaux" placeholder="reptiles" value="reptiles"></br>
...
if (isset($_POST['pays']) && (!empty($_POST['pays'])) && isset($_POST['postal']) && (!empty($_POST['postal']))){
    ...
    foreach($result as $geo){
        ...
        $annonce = "SELECT * FROM `annonce` WHERE userID IN (SELECT id FROM `utilisateur` WHERE pays,  LIKE '%" .$_POST['pays']."%'  )";
...
if (!empty($_POST)) {
    $qq = $DB->prepare('SELECT * FROM animaux ');
    $qq->execute();
    $animAll = $qq->fetchAll(PDO::FETCH_ASSOC);
    $sql = "SELECT * FROM `animaux` WHERE ".$_POST['animaux']."=1";
    $query = $DB->prepare($sql);
    $query->execute();
    $result = $query->fetchAll(PDO::FETCH_ASSOC);

    $AZER = array_intersect($animAll , $result);
    print_r($AZER);
